...
export function FieldRenderer({
  ...
  const baseInputClass = `w-full min-w-0 px-4 sm:px-5 py-3.5 sm:py-4 bg-slate-50/50 border rounded-[20px] text-[15px] font-medium text-slate-900 placeholder:text-slate-400 focus:bg-white focus:outline-none focus:ring-4 focus:ring-indigo-600/5 transition-all duration-300 ${
  ...
  return (
    <div className="space-y-2.5">
      <label className="block text-[11px] sm:text-[13px] font-semibold text-slate-600 uppercase tracking-wider ml-1">
...
function RepeaterSimple({ field, stepData, onUpdate }: {
  ...
  return (
    <div className="space-y-4">
      <label className="block text-[11px] sm:text-[13px] font-semibold text-slate-600 uppercase tracking-wider ml-1">{field.label}</label>
        ...
        {items.map((val, idx) => (
          <div key={idx} className="flex gap-3 animate-in fade-in slide-in-from-left-2 duration-300">
            <input
              ...
              placeholder={field.placeholder || ''}
              className="min-w-0 flex-1 px-4 sm:px-5 py-3.5 sm:py-4 bg-slate-50/50 border border-slate-200 rounded-[20px] text-[15px] font-medium text-slate-900 focus:bg-white focus:outline-none focus:ring-4 focus:ring-indigo-600/5 focus:border-indigo-600 transition-all shadow-sm"
            />
            <button
              onClick={() => setItems(items.filter((_, i) => i !== idx))}
              className="w-12 h-12 sm:w-14 sm:h-14 shrink-0 flex items-center justify-center bg-rose-50 text-rose-500 rounded-2xl hover:bg-rose-100 transition-all active:scale-95 shadow-sm"
            >
              <svg width="20" height="20" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <button
        ...
        className="w-full py-3 sm:py-4 px-4 sm:px-6 bg-indigo-50/50 border-2 border-dashed border-indigo-200 text-indigo-600 rounded-[20px] hover:bg-indigo-50 hover:border-indigo-400 transition-all font-bold text-[11px] sm:text-[13px] uppercase tracking-widest flex items-center justify-center gap-2"
      >
...
function RepeaterPair({ field, stepData, onUpdate }: {
  ...
  return (
    <div className="space-y-4">
      <label className="block text-[10px] sm:text-[11px] font-bold text-gray-400 uppercase tracking-widest ml-1">{field.label}s</label>
      <div className="space-y-4">
        {items.map((item, idx) => (
